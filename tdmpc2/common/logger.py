...
class VideoRecorder:
	...
	def save(self, step, key='videos/eval_video'):
		if self.enabled and len(self.frames) > 0:
			...
			fp = self._save_dir / f'{int(step)}.mp4'
			imageio.mimwrite(fp, frames, fps=self.fps)
			if self._wandb:
...
class Logger:
	...
	def log(self, d, category="train"):
		...
		if category == "train" and self._save_csv:
			...
			pd.DataFrame(self._train).to_csv(
				self._log_dir / "train.csv", header=train_keys, index=None
			)
